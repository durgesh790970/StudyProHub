class PaymentSystem {
                      ...
                      <button class="method-btn" data-method="PayTM">PayTM</button>
                        ...
                        <!-- Removed "Open in selected app" and "Choose a different method" per request -->
                                        ...
                                        <div class="transaction-area" id="transactionArea" style="display:none;">
                                            ...
                                            <input type="text" id="transactionId" placeholder="e.g. T2510262253579597269506">
    ...
    bindEvents() {
        ...
        // Sanitize transaction ID input as the user types
        const txInput = document.getElementById('transactionId');
        if (txInput) {
            txInput.addEventListener('input', () => {
                // Remove whitespace, uppercase, and keep only alphanumerics
                let v = txInput.value.replace(/\s+/g, '');
                v = v.toUpperCase();
    ...
    async verifyPayment() {
        ...
        // Validate transaction ID format: starts with 'T' followed by 22 digits
        if (!this._isValidTransactionId(transactionId)) {
            errorMsg.textContent = 'Transaction ID must be like T2510262253579597269506 (T followed by 22 digits).';
            errorMsg.style.display = 'block';
            return;
        }
    ...
    _makeOpenPdf(button, pdfId, card) {
        ...
        // Navigate to appropriate page based on pdfId pattern
        button.onclick = () => {
            try {
                // Check if this is a semester PDF
                if (pdfId.startsWith('semester-')) {
                    const semNumber = pdfId.split('-')[1];
                    if (semNumber) {
                        window.location.href = `BTecm-Semester/Semester${semNumber}.html`;
                        return;
                    }
                }
                
                // Fallback to company PDF logic
                let company = '';
                if (pdfId) {
    ...
    _isValidTransactionId(tx) {
        if (!tx || typeof tx !== 'string') return false;
        // Pattern: uppercase 'T' followed by exactly 22 digits
        const re = /^T\d{22}$/;
        return re.test(tx);
    }
    ...
    _onModalBack() {
        ...
        if (targetCard) {
            // find view notes button
            const viewNotesBtn = Array.from(targetCard.querySelectorAll('button')).find(b => b.textContent.trim().toLowerCase().includes('view notes'));
            if (viewNotesBtn) {
                viewNotesBtn.textContent = 'Open PDF';
                viewNotesBtn.onclick = () => {
                    // Show verification modal instead of direct access
                ...
                viewNotesBtn.style.background = '#2563eb';
                viewNotesBtn.style.color = '#fff';
            }
        }
